...
var sunMesh, sunFX, lensFlare;
var geometryEarth, meshEarth, meshClouds, meshLights, meshMoon, sphereAtmoMesh;
var sunPos, earthPos;
...
function initSolarSystem() {
	...
	var sunGeometry = new THREE.PlaneBufferGeometry( 1.6e6, 1.6e6 );
	 var sunMaterial = new THREE.MeshBasicMaterial( {
	 	map: THREE.ImageUtils.loadTexture( "textures/lensflare/sun_far_128.jpg" ),
	...
	var materialNormalMap = new THREE.MeshPhongMaterial( {
		specular: 0x555555,
		shininess: 10,
		map: THREE.ImageUtils.loadTexture( "textures/planets/earth_color_2048.jpg" ),
	...
	var geomSegments = 50;
	geometryEarth = new THREE.SphereGeometry( planets[2].radius, geomSegments, geomSegments );
	...
	// atmosphere fog
		...
		var atmoMaterial = new THREE.ShaderMaterial({
		 ...
		 var geometryAtmo = new THREE.SphereGeometry( planets[2].radius*1.02, 25, 25 );
		 sphereAtmoMesh = new THREE.Mesh( geometryAtmo, atmoMaterial );
		 ...
		 // clouds
	 	geometryClouds = new THREE.SphereGeometry( planets[2].radius*1.02, 25, 25 );
	...
	var materialMoon = new THREE.MeshPhongMaterial( {
		map: THREE.ImageUtils.loadTexture( "textures/planets/moon_512.jpg" )
	} );
	var moonDist = 384400;
...
function initLensFlare(){
	...
	var textureFlare_ring1 = THREE.ImageUtils.loadTexture( "textures/lensflare/lensflare5.png" );	// ring
	...
	lensFlare = new THREE.LensFlare( textureFlare_line2, 256, 0.0, THREE.AdditiveBlending, flareColor2 );
	...
	lensFlare.add( textureFlare_ring1, 20, 0.72, THREE.AdditiveBlending, flareColor );
	lensFlare.add( textureFlare_ring2, 30, 0.7, THREE.AdditiveBlending, flareColor );
...
function lensFlareUpdateCallback( object ) {
	...
	for( f = 0; f < fl; f++ ) {
			...
			if ( f > 1 ){
				...
				flare.x = object.positionScreen.x + vecX * flare.distance*1.2 - (distance - 45000) / 1e6;
			 	flare.y = object.positionScreen.y + vecY * flare.distance*1.2 - (distance - 45000) / 2e6;
				flare.scale = 2.0;
...
function sunUpdate(){
	...
	sunFX.updateCustom( 6e8 * global.DistanceScale, 228e6 * global.DistanceScale );
...
function renderSolarSystem() {
	...
	if ( curZoomStep.id < 2 ) {
		...
		if ( !meshEarth.visible ){

			meshEarth.visible = true;
			meshClouds.visible = true;
			meshLights.visible = true;
			sphereAtmoMesh.visible = true;
			meshMoon.visible = true;

			if (debug) console.log("Earth added");
		...
		if ( meshEarth.visible ){
			...
			meshEarth.visible = false;
			meshClouds.visible = false;
			meshLights.visible = false;
			sphereAtmoMesh.visible = false;
			meshMoon.visible = false;

			if (debug) console.log("Earth removed");
	...
	for ( var i = 3; i < planets.length ; i ++ ) {
		...
		//if (i != 2){		// skip Earth
			...
			if ( curZoomStep.id < 3 ) {
				...
				if ( planets[i].isInView === false ){

					scene.add( planets[i].mesh );
					planets[i].isInView = true;

					if (debug) console.log("Planet added");
				...
				if ( planets[i].isInView === true ){

					scene.remove( planets[i].mesh );
					planets[i].isInView = false;

					if (debug) console.log("Planet removed");
